...
from find_matches import distance_measures, matches_in_corpus, local_aligner_mod
import json
import time
import pickle
...
full_results = matches_in_corpus(melody_dict_LOADED, phrase_dict_LOADED, measure=local_aligner_mod)
ts = time.time()

pickle.dump(full_results, open('./O2_LAMOD_DistanceMeasures_LOADED_{0}.pkl'.format(str(ts)), "wb"))